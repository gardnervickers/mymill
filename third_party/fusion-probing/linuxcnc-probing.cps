...
description = "LinuxCNC (EMC2) with Fusion probing";
vendor = "LinuxCNC";
...
properties = {
  ...
  probeInUseChannel: {
    title      : "Probe in-use channel",
    description: "Optional LinuxCNC motion digital output (1-6) asserted while probe macros run.",
    group      : "probing",
    type       : "enum",
    values     : [
      {title:"None", id:"0"},
      {title:"Motion DOUT 0", id:"1"},
      {title:"Motion DOUT 1", id:"2"},
      {title:"Motion DOUT 2", id:"3"},
      {title:"Motion DOUT 3", id:"4"},
      {title:"Motion DOUT 4", id:"5"},
      {title:"Motion DOUT 5", id:"6"}
    ],
    value      : "0",
    scope      : "post"
  }
...
function programHasProbeOperations() {
  for (var i = 0; i < getNumberOfSections(); ++i) {
    var section = getSection(i);
    if (section.hasParameter("operation-strategy") && (section.getParameter("operation-strategy") == "probe")) {
      return true;
    }
  }
  return false;
...
function onParameter(name, value) {
  ...
  case "probe-output-work-offset":
    if (inspectionRunning) {
      probeOutputWorkOffset = 1000;
    } else if (value > PROBE_MAX_WORK_OFFSET) {
      error(localize("Probe output work offset out of range. This post supports G54..G59.3 only."));
    } else {
...
function doInspectionCommand(command) {
  var value = String(command).toLowerCase();
  switch (value) {
  case "start":
  case "on":
  case "begin":
...
function onManualNC(command, value) {
  switch (command) {
  case COMMAND_ACTION:
    var commands = String(value).toLowerCase().split(/[,= ]+/);
    if (commands[0] == "inspection" && commands.length > 1) {
...
function isProbeCycle(cycleName) {
  return cycleName && (cycleName.indexOf("probing-") === 0);
}
...
function TurnOutputOn(channel) {
  ...
  if (channelNumber < 1 || channelNumber > 6) {
    error(localize("This post is configured for LinuxCNC motion digital outputs P0..P5 only."));
    return;
  }
  writeBlock("M64", pFormat.format(channelNumber - 1));
}
...
function TurnOutputOff(channel) {
  ...
  writeBlock("M65", pFormat.format(channelNumber - 1));
}

function ShowProbeHeader() {
  if (isProbeOperation() && inspectionRunning) {
    var comment = getParameter("operation-comment", "Probe");
...
function writeProbeMacroCall(x, y, z, F, cycleArgs) {
  var args = [
    formatSubroutineCall(cycleType),
  ...
  ].concat(cycleArgs, getProbingArguments(cycle));

  writeComment(cycleType);
  ShowProbeHeader();
  TurnOutputOn(getProperty("probeInUseChannel"));
  writeBlock.apply(undefined, args);
  TurnOutputOff(getProperty("probeInUseChannel"));

  // Probing macros can leave modal state changed; force the next move to restate it.
  forceXYZ();
  gMotionModal.reset();
}

function emitProbeCycle(x, y, z, F) {
  if (!isProbeOperation() || !isProbeCycle(cycleType)) {
    return false;
  }
...
function onCommand(command) {
  ...
  case COMMAND_LOAD_TOOL:
    writeToolBlock("T" + toolFormat.format(tool.number), useM06 ? mFormat.format(6) : "");
